...
for opt in "${OPTIONS[@]}"; do
    case "$opt" in
        ...
        --s)
			...
			paste -d' ' <(grep '^DE-Score:' *.SiteSaturation.TotalFrequencies.txt | sed 's/.SiteSaturation.TotalFrequencies.txt:DE-Score:[[:space:]]*/ /') <(grep '^Critical DE-Score:' *.SiteSaturation.TotalFrequencies.txt | sed 's/.*Critical DE-Score:[[:space:]]*/ /') > "$3.DEScore.fastaSummary.txt"				Genewise.DEScoreReport.r $3.DEScore.fastaSummary.txt
            elif [ "$ARG1" = "dna" ]; then
